import { TextField } from '@mui/material'; //from material ui site - components API -> TextField ->first link copy,paste
import Stack from '@mui/material/Stack'; // from same site- stack - basis -link copy,paste
...
function App(){
  ...
  const getValidate = (e)=>{ //destructuring
    const {name ,value} = e.target
    // console.log(name ,value);
    // console.log(typeof(value));
    // console.log(!!value.match(/^[0-9]+$/)) //returns array or null -
    if(!!value.match(/^[0-9]*.?[0-9]+$/)){ //  !! - to change all expression as boolean
    ...
    <div style={{height:'100vh'}} className='d-flex justify-content-center align-items-center w-100 bg-dark'>
      <div className='bg-light p-5 rounded' style={{width:'500px'}}>
        ...
        <form className='mt-5' onSubmit={handleCalculate}>
            ...
            {!isPrinciple &&  //truthy operator- conditing rendering
              <div>
            ...
            {!isRate &&  //truthy operator- conditing rendering
              <div>
              <p className='text-danger'>Invalid Input</p>
            </div>}
            ...
            {!isYear &&  //truthy operator- conditing rendering
              <div>
              <p className='text-danger'>Invalid Input</p>
            </div>}
